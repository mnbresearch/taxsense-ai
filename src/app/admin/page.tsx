        ...
        <>
          ...
          {leads && leads.some((l) => l.plan) && (() => {
            ...
            for (const l of leads) {
              if (!l.plan) continue;
              const b = l.status === "active" ? buckets.active : buckets.lead;
              const mo = /₹([\d,]+)\/mo/.exec(l.plan);
              const ret = /₹([\d,]+)\/return/.exec(l.plan);
              const yr = /₹([\d,]+)\/yr\)/.exec(l.plan);
              if (mo) b.mrr += Number(mo[1].replace(/,/g, ""));
              else if (ret) b.oneTime += Number(ret[1].replace(/,/g, ""));
              else if (yr) b.mrr += Math.round(Number(yr[1].replace(/,/g, "")) / 12);
            }
            return (
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <div className="rounded-xl border border-green-200 bg-green-50/70 p-5">
                  <h2 className="font-semibold text-green-900">Active revenue <span className="text-xs font-normal text-green-700">customers you've marked ₹ Paid</span></h2>
                  <div className="mt-2 flex gap-6">
                    <div><span className="text-2xl font-bold text-green-700">₹{buckets.active.mrr.toLocaleString("en-IN")}</span><span className="ml-1 text-xs text-green-800">MRR</span></div>
                    <div><span className="text-2xl font-bold text-green-700">₹{buckets.active.oneTime.toLocaleString("en-IN")}</span><span className="ml-1 text-xs text-green-800">one-time</span></div>
                  </div>
                </div>
                <div className="rounded-xl border border-brand-200 bg-brand-50/60 p-5">
                  <h2 className="font-semibold">Pipeline <span className="text-xs font-normal text-stone-500">plan requests awaiting your call</span></h2>
                  <div className="mt-2 flex gap-6">
                    <div><span className="text-2xl font-bold text-brand-700">₹{buckets.lead.mrr.toLocaleString("en-IN")}</span><span className="ml-1 text-xs text-stone-500">potential MRR</span></div>
                    <div><span className="text-2xl font-bold text-brand-700">₹{buckets.lead.oneTime.toLocaleString("en-IN")}</span><span className="ml-1 text-xs text-stone-500">one-time (filings)</span></div>
                  </div>
                </div>
              </div>
            );
